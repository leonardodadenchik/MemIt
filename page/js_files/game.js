//global variables
let host = (location.origin.replace(/^http/, "ws") + ":8814/").replace(
  ":3000",
  "",
);
...
function start_game() {
  if (game_data.player_id === 1) {
    myWs.send(
      JSON.stringify({ content: "start_game", room_code: game_data.room_code }),
    );
  } else {
...
function send_room_data() {
  ...
  go_to_connection();
  myWs.send(
    JSON.stringify({ content: "room_creation", room_settings: room_settings }),
  );
}
...
myWs.onmessage = function (jsonMessage) {
  ...
  if (jsonMessage.content === "message") {
    ...
    document.getElementById("card_status").innerHTML =
      "You have not chosen a card yet";
