...
const {
    differentNums, generatorNotExist, cardGen, delete_player,
} = require("./module_func");
                        ...
                        if (room) {
                            if (room.players.length === room.playersCount) {
                                ...
                                wsClient.send(JSON.stringify({
                                    ...
                                    room_code: jsonMessage.code
                                }))
                                room.players.push({
                                    name: jsonMessage.name, wsClient: wsClient, card: "selecting...",
                                })
                    ...
                    for (let room of rooms) {
                        if ((room.code == jsonMessage.room_code) & (room.step == 0)) {
                            ...
                            for (let player of room.players) {
                                ...
                                player.wsClient.send(JSON.stringify({
